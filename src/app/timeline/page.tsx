...
export default function TimelinePage() {
  ...
  const [explanation, setExplanation] = useState<ExplainConceptOutput | null>(
    null
  );
  ...
  const handleExplainClick = (event: TimelineEvent) => {
    ...
    startTransition(async () => {
      const result = await explainConcept({
        concept: `${event.title} (${event.year})`,
      });
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-slate-50 via-amber-50 to-orange-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden">
        <div className="absolute top-20 left-10 w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 bg-amber-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob"></div>
        <div className="absolute bottom-20 right-10 w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 bg-rose-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000"></div>
      </div>
      ...
      <main className="flex-grow container mx-auto px-4 py-12 md:py-20 relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="bg-white/90 backdrop-blur-xl shadow-2xl border-2 border-amber-200/50 mb-12">
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                <span className="font-bold text-amber-800 text-lg min-w-[60px]">
                  {range[0]}
                </span>
                <Slider
                  min={minYear}
          ...
          <div className="relative mx-auto max-w-4xl">
            ...
            <div
              className="absolute left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-amber-300/80 via-orange-300/60 to-rose-300/80 rounded-full shadow-lg"
              aria-hidden
            />
                ...
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      } w-20 h-20 opacity-10 group-hover:opacity-20 transition-opacity`}
                    >
                      <div
                        className={`absolute ${
                          isLeft ? "top-2 right-2" : "top-2 left-2"
                    ...
                    <CardHeader className="relative z-10">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <p className="text-3xl font-extrabold text-amber-700 font-headline">
                          {event.year}
                        </p>
                      </div>
                    ...
                    <CardFooter className="border-t border-amber-100 bg-gradient-to-r from-amber-50/50 to-orange-50/50">
                      ...
                      >
                        <span className="flex items-center gap-2">
                          <svg
                            className="w-5 h-5 group-hover/button:rotate-12 transition-transform"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
            ...
            {filteredEvents.length === 0 && (
              <div className="text-center py-20 animate-fade-in">
                ...
                <p className="text-sm text-foreground/50 mt-2">
                  Hãy điều chỉnh khoảng thời gian để xem thêm sự kiện
                </p>
              </div>
            )}
          </div>
