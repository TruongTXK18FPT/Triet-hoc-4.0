...
import { Loader2, LogOut, Save, BookOpen, FileText, Star, MessageSquare, Map, TrendingUp, User, Eye } from 'lucide-react';
    ...
    if (session?.user) {
      ...
      startTransition(async () => {
        ...
        if (reviewRes.ok) {
          ...
          const userReviews = reviewData.reviews?.filter((r: any) => r.user.email === session?.user?.email) || [];
        ...
        <div className="container mx-auto px-4 h-full flex items-end pb-8 relative z-10">
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-3xl font-bold shadow-xl">
              {session?.user?.name?.charAt(0)?.toUpperCase() || <User className="h-10 w-10" />}
            </div>
            <div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <StatCard icon={BookOpen} label="Quizzes" value={stats.quizzes} color="from-purple-400 to-pink-500" />
              <StatCard icon={Map} label="Roadmaps" value={stats.roadmaps} color="from-green-400 to-emerald-500" />
              <StatCard icon={Star} label="Reviews" value={stats.reviews} color="from-yellow-400 to-orange-500" />
              <StatCard icon={FileText} label="Posts" value={stats.posts} color="from-blue-400 to-cyan-500" />
              <StatCard icon={MessageSquare} label="Comments" value={stats.comments} color="from-rose-400 to-red-500" />
            </div>
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <h4 className="font-semibold text-amber-900 group-hover:text-amber-700">{roadmap.title}</h4>
                            {roadmap.description && (
                              <p className="text-sm text-slate-600 mt-1">{roadmap.description}</p>
                            )}
                            <p className="text-xs text-slate-500 mt-2">
                              {roadmap.items?.length || 0} mục • Tạo ngày {new Date(roadmap.createdAt).toLocaleDateString('vi-VN')}
                            </p>
                          </div>
                  ...
                  <div className="space-y-4">
                    {reviews.map((review: any) => (
                      ...
                      >
                        ...
                        {review.title && (
                          <h4 className="font-semibold text-blue-900 mb-1">{review.title}</h4>
                        )}
          ...
          <div className="space-y-6">
            <Card className="bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200 shadow-lg">
              <CardHeader>
                <CardTitle className="text-xl text-amber-900">Gợi ý dành cho bạn</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm text-amber-800/90">
                <div className="flex gap-3 items-start">
                  ...
                  <p>Hoàn thiện hồ sơ để AI cá nhân hoá lộ trình học tốt hơn.</p>
                </div>
                <div className="flex gap-3 items-start">
                  <div className="h-8 w-8 rounded-full bg-amber-200 flex items-center justify-center flex-shrink-0">
            ...
            <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 shadow-lg">
              <CardHeader>
                <CardTitle className="text-xl text-blue-900">Thành tích</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
                  <span className="text-sm font-medium text-blue-900">Tổng hoạt động</span>
                  <span className="font-bold text-lg text-blue-700">
                    {stats.quizzes + stats.roadmaps + stats.reviews + stats.posts + stats.comments}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
                  <span className="text-sm font-medium text-blue-900">Cấp độ</span>
                  <span className="font-bold text-lg text-blue-700">
                    {stats.quizzes + stats.posts > 10 ? '🎓 Chuyên gia' : stats.quizzes + stats.posts > 5 ? '📚 Trung cấp' : '🌱 Mới bắt đầu'}
                  </span>
                </div>
              </CardContent>
            </Card>
      ...
      <Dialog open={!!selectedRoadmap} onOpenChange={() => setSelectedRoadmap(null)}>
        <DialogContent className="max-w-6xl max-h-[90vh] w-[95vw]">
          ...
          <div className="h-[70vh]">
            {selectedRoadmap && (
              <RoadmapFlow 
                lines={selectedRoadmap.items?.map((item: any) => item.title) || []} 
  ...
  return (
    <Card className="bg-white/90 backdrop-blur border-slate-200 hover:shadow-lg transition-all hover:-translate-y-1">
      <CardContent className="p-4 text-center">
        <div className={`h-12 w-12 mx-auto mb-2 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center`}>
